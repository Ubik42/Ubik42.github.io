...
`),x=g.split(`
`);for(s=i=0;i<S.length&&!S[i].includes("DetermineComponentFrameRoot");)i++;for(;s<x.length&&!x[s].includes("DetermineComponentFrameRoot");)s++;if(i===S.length||s===x.length)for(i=S.length-1,s=x.length-1;1<=i&&0<=s&&S[i]!==x[s];)s--;for(;1<=i&&0<=s;i--,s--)if(S[i]!==x[s]){if(i!==1||s!==1)do if(i--,s--,0>s||S[i]!==x[s]){var j=`
...
`,Op={fathom:{zh:oE,en:lE},"house-of-vampires":{zh:cE,en:uE},"through-other-eyes":{zh:mE,en:hE},"retro-arcade":{zh:dE,en:fE},"digital-twin":{zh:sE,en:rE}},gE=[{id:"noemancer",title:"最最 AI 友好的高性能引擎",category:"engine-games",categoryLabel:{zh:"自研游戏引擎",en:"Custom game engine"},summary:{zh:"从原生 Editor、C# Gameplay、资产 Cook 到 D3D12/Vulkan 商业 Raster 和独立 Player 的 C++20 游戏引擎。",en:"A C++20 game engine spanning a native editor, C# gameplay, asset cooking, D3D12/Vulkan commercial raster rendering, and standalone players."},cover:"/media/repositories/major-updates/noemancer-sponza-atrium.webp",tags:["C++20","D3D12 / Vulkan","C#","MCP"],repositoryUrl:"https://github.com/Ubik42/Noemancer",story:{zh:`# 最最 AI 友好的高性能引擎
